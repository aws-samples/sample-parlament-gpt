...
| S6 | Impersonating the agent/Gateway (rogue MCP endpoint) | ✅ (as deployed) | Gateway auth is SigV4 with the runtime's IAM role (`GATEWAY_AUTH_MODE=iam`); the URL comes from the CDK output, not from user input (`agent/src/parlamentgpt_agent/gateway.py`). The code also carries a **dormant Cognito `client_credentials` fallback** (`GATEWAY_AUTH_MODE=cognito`) that no stack deploys; enabling it moves this row's assumptions — the operator then owns a client secret, token lifetimes and the token endpoint as additional attack surface. |
| S7 | Post-logout session reuse | ⚠️ partial (best-effort revocation) | Sign-out records a revocation marker (`sk=REVOCATION`, own TTL) and every authenticated route rejects tokens issued at or before it — `lib/authGuard.ts` (`requireUser`), used by `/api/ask`, `/api/sessions*`, `/api/settings`; 7 tests incl. the same-second edge case (`test/authGuard.test.ts`). Residual 1: the check **fails open** — if the revocation store is unreachable, a cryptographically valid token is accepted for its remaining lifetime (up to 12 h), a deliberate availability trade-off (`authGuard.ts`). Residual 2: the page shell (Edge middleware, signature-only) still renders for a revoked token — it carries no user data. Detection: both degraded paths (fail-open read, failed revocation write) emit fixed log phrases feeding the `ParlamentGPT/RevocationDegraded` metric (`frontend-stack.ts`), so a store outage during which revocation is effectively off is alarmable, not silent. Shortening the 12 h token lifetime remains a production decision (README #2). |
